...
{
	screen= (uint8_t *)ps_malloc(200 * 640);
...
void DISPLAY_::draw_screen0_normal(uint16_t src)
{
...
void DISPLAY_::draw_screen0_wide(uint16_t src)
{
			...
			if(src_t == cursor) {
				...
				if((bp == 0x40 && (cblink & 8)) || (bp == 0x60 && (cblink & 0x10))) {
					for(int i = (regs[10] & 7); i < 8; i++) {
						...
						memset(screen + (y + i) * 640 + (x << 4), 7, 16);
					}
				}
			}
			src_t = (src_t & 0x3800) | ((src_t + 1) & 0x7ff);
...
void DISPLAY_::draw_screen1_normal(uint16_t src)
{
...
void DISPLAY_::draw_screen1_wide(uint16_t src)
{
...
void DISPLAY_::draw_screen2_normal(uint16_t src)
{
...
void DISPLAY_::draw_screen2_wide(uint16_t src)
{
	// screen 0, wide char (36chars)
	uint16_t src_t = src & 0x7ff;
	uint8_t c_b = mode & 7;
...
void DISPLAY_::draw_screen15_normal(uint16_t src)
{
	// screen 2, normal char (80chars)
	uint16_t src_t = src & 0x7ff;
	uint8_t c_b = mode & 7;
...
void DISPLAY_::draw_screen15_wide(uint16_t src)
{
	// screen 0, normal char (80chars)
	uint16_t src_t = src & 0x7ff;
	uint8_t c_b = mode & 7;
