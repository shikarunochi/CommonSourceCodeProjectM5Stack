...
{
	ipl = (uint8_t*)ps_malloc(0x8000);	// ipl rom (32k)
	basic = (uint8_t*)ps_malloc(0x4000);	// basic rom (16k)
	cart = (uint8_t*)ps_malloc(0x8000);	// cartridge (32k)
	
	wdmy = (uint8_t*)ps_malloc(0x1000);
	rdmy = (uint8_t*)ps_malloc(0x1000);
	memset(ipl, 0xff, 0x8000);
	memset(basic, 0xff, 0x4000);
