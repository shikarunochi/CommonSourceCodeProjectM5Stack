...
void SUB::initialize()
{
	sub1 = (uint8_t*)ps_malloc(0x1000);
	sub2 = (uint8_t*)ps_malloc(0x1000);
	sub3 = (uint8_t*)ps_malloc(0x1000);

	vram_b = (uint8_t*)ps_malloc(0x4000);
	vram_r = (uint8_t*)ps_malloc(0x4000);
	vram_g = (uint8_t*)ps_malloc(0x4000);

	screen = (uint8_t*)ps_malloc(400 * 640);

	// init memory
	memset(sub1, 0xff, 0x1000);
	memset(sub2, 0xff, 0x1000);
	memset(sub3, 0xff, 0x1000);
	memset(rdmy, 0xff, sizeof(rdmy));
	memset(ram, 0, sizeof(ram));
	memset(vram_b, 0, 0x4000);
	memset(vram_r, 0, 0x4000);
	memset(vram_g, 0, 0x4000);
	
	FILEIO* fio = new FILEIO();
	if(fio->Fopen(create_local_path(_T("SUB1.ROM")), FILEIO_READ_BINARY)) {
		fio->Fread(sub1, 0x1000, 1);
		fio->Fclose();
	}
	if(fio->Fopen(create_local_path(_T("SUB2.ROM")), FILEIO_READ_BINARY)) {
		fio->Fread(sub2, 0x1000, 1);
		fio->Fclose();
	}
	if(fio->Fopen(create_local_path(_T("SUB3.ROM")), FILEIO_READ_BINARY)) {
		fio->Fread(sub3, 0x1000, 1);
		fio->Fclose();
	}
	delete fio;
...
void SUB::write_io8(uint32_t addr, uint32_t data)
{
	...
	case P_A:
		...
		if((pa & 0x20) && !(data & 0x20)) {
			...
			memset(vram_b, (fore_color & 1) ? 0xff : 0, 0x4000);
			memset(vram_r, (fore_color & 2) ? 0xff : 0, 0x4000);
			memset(vram_g, (fore_color & 4) ? 0xff : 0, 0x4000);
		}
...
//	emu->set_vm_screen_lines((ymax > 400) ? 400 : ymax);
					...
					if(src == cursor && (regs[8] & 0xc0) != 0xc0) {
						...
						if(bp == 0 || (bp == 0x40 && (cblink & 8)) || (bp == 0x60 && (cblink & 0x10))) {
							...
							for(int l = (regs[10] & 0x1f); l < lmax; l++) {
								...
								memset(screen + (y + l) * 640 + x, cursor_color, 16);
							}
						}
					}
					src = (src + 8) & 0x3fff;
					...
					if(src == cursor && (regs[8] & 0xc0) != 0xc0) {
						...
						if(bp == 0 || (bp == 0x40 && (cblink & 8)) || (bp == 0x60 && (cblink & 0x10))) {
							...
							for(int l = (regs[10] & 0x1f); l < lmax; l++) {
								...
								memset(screen + (y + l) * 640 + x, cursor_color, 8);
							}
						}
					}
					src = (src + 8) & 0x3fff;
	...
	if(ymax > 200) {
		...
		for(int y = 0; y < 200; y++) {
			...
			uint8_t* src1 = screen + y * 2 * 640;
			uint8_t* src2 = screen + (y * 2 + 1) * 640;
			
			for(int x = 0; x < 320; x++) {
				scrntype_t color1 = palette_pc[src1[x*2] & 7];
				scrntype_t color2 = palette_pc[src1[x*2 + 1] & 7];
				scrntype_t color3 = palette_pc[src2[x*2] & 7];
				scrntype_t color4 = palette_pc[src2[x*2 + 1] & 7];
				uint8_t rr =(R_OF_COLOR(color1) + R_OF_COLOR(color2) + R_OF_COLOR(color3) + R_OF_COLOR(color4)) / 4;
				uint8_t gg =(G_OF_COLOR(color1) + G_OF_COLOR(color2) + G_OF_COLOR(color3) + G_OF_COLOR(color4)) / 4;
				uint8_t bb =(B_OF_COLOR(color1) + B_OF_COLOR(color2) + B_OF_COLOR(color3) + B_OF_COLOR(color4)) / 4;
				dest[x] = RGB_COLOR(rr,gg,bb);
			}
		}
		...
		for(int y = 0; y < 200; y++) {
			...
			uint8_t* src = screen +  y * 640;

			//640 -> 320
			for(int x = 0; x < 320; x++) {
				scrntype_t color1 = palette_pc[src[x*2] & 7];
				scrntype_t color2 = palette_pc[src[x*2 + 1] & 7];
				uint8_t rr =(R_OF_COLOR(color1) + R_OF_COLOR(color2)) / 2;
				uint8_t gg =(G_OF_COLOR(color1) + G_OF_COLOR(color2)) / 2;
				uint8_t bb =(B_OF_COLOR(color1) + B_OF_COLOR(color2)) / 2;
				dest0[x] = RGB_COLOR(rr,gg,bb);
			}
			//if(config.scan_line) {
...
bool SUB::process_state(FILEIO* state_fio, bool loading)
{
	...
	state_fio->StateArray(vram_b, 0x4000, 1);
	state_fio->StateArray(vram_r, 0x4000, 1);
	state_fio->StateArray(vram_g, 0x4000, 1);
