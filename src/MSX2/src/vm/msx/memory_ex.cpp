...
Serial.println(file_size,HEX);
		if(file_size <= 0x2000) {
			Serial.println("ROM SIZE < 0x2000:8K");
			// 8KB: 00000000
			...
			Serial.println("ROM SIZE < 0x4000:16K");
			// 16KB: 01010101
			fio->Fread(rom, 0x4000, 1);
			memcpy(rom + 0x4000, rom, 0x4000);
			memcpy(rom + 0x8000, rom, 0x8000);
		} else if(file_size <= 0x8000) {
			Serial.println("ROM SIZE < 0x8000:32K");
			// 32KB: 01012323
			...
			if(file_size > 0x10000) {
				Serial.printf("MEGA ROM:fileSize:%d\n", file_size);
				if(file_size > MEGAROM_SIZE) file_size = MEGAROM_SIZE;
				...
				Serial.println("ROM SIZE < 0x10000:64K");
				// 64KB: 01234567
...
void SLOT_CART::initialize()
{
    	...
    	rdmy = (uint8_t*)ps_malloc(0x2000);
#ifdef USE_MEGAROM
		//rom = (uint8_t*)ps_malloc(1024*1024*4);
		rom = (uint8_t*)ps_malloc(MEGAROM_SIZE);
#else
		rom = (uint8_t*)ps_malloc(0x10000);
		Serial.println("SLOT_CART INITIALIZE OK");
#endif
	memset(rdmy, 0xff, 0x2000);
	close_cart();
}
...
{	
	wdmy = (uint8_t*)ps_malloc(0x2000);
	rdmy = (uint8_t*)ps_malloc(0x2000);
	ram = (uint8_t*)ps_malloc(MAPPERRAM_SIZE);
	
	memset(ram, (uint8_t)0xff, MAPPERRAM_SIZE);
	/*close_cart();*/
	for(int i = 0; i < 4; i++) {
		mapper[i] = i;
	}
...
bool SLOT_MAPPERRAM::process_state(FILEIO* state_fio, bool loading)
{
	...
	state_fio->StateArray(ram, MAPPERRAM_SIZE, 1);
	state_fio->StateArray(mapper, sizeof(mapper), 1);
