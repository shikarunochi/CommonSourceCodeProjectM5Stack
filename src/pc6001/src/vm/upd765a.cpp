...
{
	Serial.println("UPD765A initialize");
	// initialize d88 handler
	for(int i = 0; i < 4; i++) {
		Serial.printf("DISK:%d",i);
		disk[i] = new DISK(emu);
		Serial.printf("NEW OK");
		disk[i]->set_device_name(_T("%s/Disk #%d"), this_device_name, i + 1);
	}
	Serial.println("DISK OK");
	...
	// initialize fdc
	buffer = (uint8_t*)ps_malloc(0x8000);
	...
	Serial.println("DISK Inicialize OK");
}
...
bool UPD765A::process_state(FILEIO* state_fio, bool loading)
{
	...
	state_fio->StateArray(buffer, 0x8000, 1);
