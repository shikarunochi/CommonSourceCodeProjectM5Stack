...
#elif defined(_M5Stack)
	return SD.exists(String(file_path));
#else
...
#elif defined(_M5Stack)
	return SD.remove(String(file_path));
#else
...
bool FILEIO::Fopen(const _TCHAR *file_path, int mode)
{
	...
	case FILEIO_WRITE_BINARY:
		return (file = SD.open(file_path, "w"));
	...
	case FILEIO_WRITE_APPEND_ASCII:
		return (file = SD.open(file_path, "a"));
...
char *FILEIO::Fgets(char *str, int n)
{
	...
	if(buf.length() != 0){
		memcpy(str, buf.c_str(),buf.length());
		return str;
	}else{
		return 0;
	}
...
int FILEIO::Fseek(long offset, int origin)
{
	...
	if(file == true) {
		switch(origin) {
		case FILEIO_SEEK_CUR:
			return file.seek(offset, SeekCur);
		case FILEIO_SEEK_END:
			return file.seek(offset, SeekEnd);
		case FILEIO_SEEK_SET:
