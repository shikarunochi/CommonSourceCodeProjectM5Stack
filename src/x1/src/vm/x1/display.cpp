...
{
	vram_t = (uint8_t *)ps_malloc(0x800);
	vram_a = (uint8_t *)ps_malloc(0x800);
	pcg_b = (uint8_t *)ps_malloc(256*8);
	pcg_r = (uint8_t *)ps_malloc(256*8);
	pcg_g = (uint8_t *)ps_malloc(256*8);
	font = (uint8_t *)ps_malloc(0x800);
	//kanji = (uint8_t *)ps_malloc(0x4bc00);
	text = (uint8_t *)ps_malloc(200*(640+8));
	cg = (uint8_t *)ps_malloc(200*640);
	pri_line = (uint8_t *)ps_malloc(200 * 8 * 8);
	...
	memset(vram_t, 0, 0x800);
	memset(vram_a, 0, 0x800);
#ifdef _X1TURBO_FEATURE
	memset(vram_k, 0, sizeof(vram_k));
#endif
	memset(pcg_b, 0, 256*8);
	memset(pcg_r, 0, 256*8);
	memset(pcg_g, 0, 256*8);
#ifdef _X1TURBO_FEATURE
...
void DISPLAY_::write_io8(uint32_t addr, uint32_t data)
{
	switch(addr & 0xff00) {
	case 0x0e00:
...
//				zpal_num = get_zpal_num(addr, data);
	...
	case 0x1500:
		get_cur_pcg(addr);
		*(pcg_b + cur_code * 8 + cur_line) = data;
#ifdef _X1TURBO_FEATURE
	...
	case 0x1600:
		get_cur_pcg(addr);
		*(pcg_r + cur_code * 8 + cur_line) = data;
#ifdef _X1TURBO_FEATURE
	...
	case 0x1700:
		get_cur_pcg(addr);
		*(pcg_g + cur_code * 8 + cur_line) = data;
#ifdef _X1TURBO_FEATURE
...
uint32_t DISPLAY_::read_io8(uint32_t addr)
{
	switch(addr & 0xff00) {
	case 0x0e00:
...
//		return priority;
	...
	case 0x1500:
		get_cur_pcg(addr);
		return *(pcg_b + cur_code * 8 + cur_line);
	case 0x1600:
		get_cur_pcg(addr);
		return *(pcg_r + cur_code * 8 + cur_line);
	case 0x1700:
		get_cur_pcg(addr);
		return *(pcg_g + cur_code * 8 + cur_line);
#ifdef _X1TURBOZ
	case 0x1f00:
		switch(addr) {
		case 0x1fb0:
...
void DISPLAY_::event_frame()
{
	...
	memset(text, 0, 200 * (640+8));
	memset(cg, 0, 200*640);
	memset(pri_line, 0, 200 * 8 * 8);
#ifdef _X1TURBOZ
...
uint8_t DISPLAY_::get_cur_font(uint32_t addr)
{
#ifdef _X1TURBO_FEATURE
	if(mode1 & 0x20) {
		// wait next blank
...
void DISPLAY_::get_cur_pcg(uint32_t addr)
{
#ifdef _X1TURBO_FEATURE
	if(mode1 & 0x20) {
		// wait next blank
...
void DISPLAY_::get_cur_code_line()
{
//#ifdef _X1TURBO_FEATURE
...
#endif
		draw_cg(v, 0);
		memcpy( pri_line + v * 8 * 8 + 0 * 8 + 0, &pri[0][0], sizeof(pri));
//	} else {
			...
			for(int y = 0; y < 200; y++) {
				...
				if(aen_line[y]) {
					...
					for(int x = 0; x < 320; x++) {
						...
						dest0[x] =  palette_pc[*(pri_line + y * 8 * 8 + *(src_cg + x) * 8 + *(src_text + x))];
#endif
					}
#ifdef _X1TURBOZ
				}
...
#else
						dest0[x] =  palette_pc_half
								[*(pri_line + y * 8 * 8 + *(src_cg + x*2) * 8 + *(src_text + x*2))]
								[*(pri_line + y * 8 * 8 + *(src_cg + x*2 + 1) * 8 + *(src_text + x*2 + 1))];
#endif
					}
#ifdef _X1TURBOZ
				}
...
#endif
				pattern_b = pcg_b + code * 8;
				pattern_r = pcg_r + code * 8;
				pattern_g = pcg_g + code * 8;
#ifdef _X1TURBO_FEATURE
				shift = (mode1 & 1) ? 1 : 0;
			}
#endif
...
void DISPLAY_::draw_cg(int line, int plane)
{
	int width = column40 ? 40 : 80;
...
void DISPLAY_::write_kanji(uint32_t addr, uint32_t data)
{
	switch(addr) {
	case 0xe80:
...
uint32_t DISPLAY_::read_kanji(uint32_t addr)
{
	switch(addr) {
	case 0xe80:
		if(kaddr & 0xff00) {
...
uint16_t DISPLAY_::jis2adr_x1(uint16_t jis)
{
	uint16_t jh, jl, adr;
...
uint32_t DISPLAY_::adr2knj_x1(uint16_t adr)
{
	uint16_t jh, jl, jis;
...
uint32_t DISPLAY_::jis2knj(uint16_t jis)
{
...
uint16_t DISPLAY_::jis2sjis(uint16_t jis)
{
	uint16_t c1, c2;
...
bool DISPLAY_::process_state(FILEIO* state_fio, bool loading)
{
	...
	state_fio->StateArray(vram_t, 0x800, 1);
	state_fio->StateArray(vram_a, 0x800, 1);
#ifdef _X1TURBO_FEATURE
	state_fio->StateArray(vram_k, sizeof(vram_k), 1);
#endif
	state_fio->StateArray(pcg_b, 256 * 8, 1);
	state_fio->StateArray(pcg_r, 256 * 8, 1);
	state_fio->StateArray(pcg_g, 256 * 8, 1);
#ifdef _X1TURBO_FEATURE
