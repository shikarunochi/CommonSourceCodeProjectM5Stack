...
VM::VM(EMU* parent_emu) : VM_TEMPLATE(parent_emu)
{
	...
	sound_type = 1;//config.sound_type;
	// create devices
...
//	io->set_iovalue_single_r(0x1ff0, 0x00);
	...
	for(DEVICE* device = first_device; device; device = device->next_device) {
		Serial.printf("initialize:%s",device->get_device_name());
		device->initialize();
		Serial.println(":OK!");
	}
...
void VM::reset()
{
	// reset all devices
	for(DEVICE* device = first_device; device; device = device->next_device) {
		Serial.printf("reset:%s",device->get_device_name());
		device->reset();
		Serial.println(":OK!");
	}
	...
	for(DEVICE* device = cpu; device; device = device->get_context_child()) {
		Serial.printf("force reset:%s",device->get_device_name());
		device->reset();
		Serial.println(":OK!");
	}
#else
