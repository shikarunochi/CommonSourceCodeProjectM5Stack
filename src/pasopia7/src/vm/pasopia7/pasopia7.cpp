...
VM::VM(EMU* parent_emu) : VM_TEMPLATE(parent_emu)
{
	...
	pio2->set_context_port_a(dummy, SIG_SN76489AN_MUTE, 0x02, 0);
	...
	//display->set_vram_ptr(memory->get_vram());//need initialize
	display->set_pal_ptr(memory->get_pal());
	display->set_regs_ptr(crtc->get_regs());
	floppy->set_context_fdc(fdc);
	iobus->set_context_io(io);
	//iobus->set_ram_ptr(memory->get_ram());//need initialize
	...
	io->set_iomap_single_w(0x3a, dummy);
	//io->set_iomap_single_w(0x3b, psg1);
	io->set_iomap_single_w(0x3b, dummy);
