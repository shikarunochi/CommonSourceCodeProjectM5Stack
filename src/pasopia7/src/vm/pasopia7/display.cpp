...
void DISPLAY_::draw_text_normal(uint16_t src)
{
	...
	for(int y = 0; y < 200; y += 8) {
		for(int x = 0; x < 80; x++, dstx++, src = (src + 8) & 0x3ff8, src_t = (src_t + 8) & 0x3fff) {
			...
			for(int l = 0; l < 8; l++) {
				...
				uint8_t* d = screen + ((dsty << 3) + l) * 640 + (_dstx << 3);
			...
			if(src == cursor) {
				...
				if(bp == 0 || (bp == 0x40 && (cblink & 8)) || (bp == 0x60 && (cblink & 0x10))) {
					for(int i = (regs[10] & 7); i < 8; i++) {
						...
						memset(screen +((dsty << 3) + i ) * 640 + (_dstx << 3), 7, 8);
					}
				}
			}
		}
...
void DISPLAY_::draw_text_wide(uint16_t src)
{
	...
	for(int y = 0; y < 200; y += 8) {
		for(int x = 0; x < 40; x++, dstx++, src = (src + 8) & 0x3ff8, src_t = (src_t + 8) & 0x3fff) {
			...
			for(int l = 0; l < 8; l++) {
				...
				uint8_t* d = screen + ((dsty << 3) + l) * 640 + (_dstx << 4);
			...
			if(src == cursor) {
				...
				if(bp == 0 || (bp == 0x40 && (cblink & 8)) || (bp == 0x60 && (cblink & 0x10))) {
					for(int i = (regs[10] & 7); i < 8; i++) {
						...
						memset(screen + ((dsty << 3) + i) * 640 + (_dstx << 4), 7, 16);
					}
				}
			}
		}
...
void DISPLAY_::draw_fine_normal(uint16_t src)
{
...
void DISPLAY_::draw_fine_wide(uint16_t src)
{
...
void DISPLAY_::draw_text_lcd(uint16_t src)
{
...
void DISPLAY_::draw_fine_lcd(uint16_t src)
{
	// fine graph mode, normal char (80chars)
	for(int y = 0, yy = 0; y < 64; y += 8, yy++) {
		int src_g = (src + 640 * (yy & 3) + (yy & 4 ? 8 : 0)) & 0x3ff8;
