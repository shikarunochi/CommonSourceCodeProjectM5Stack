/*
	...
	M5Stack version.
	modified by shikarunochi 2020.05.23 -
...
void MEMORY::initialize()
{
	...
	rom = (uint8_t*)ps_malloc(sizeOfROM);
	memset(rom, 0xff, sizeOfROM);


	cram = (uint8_t*)ps_malloc(0x800);
	memset(cram, 0, 0x800);

	aram = (uint8_t*)ps_malloc(0x800);
	memset(aram, 0, 0x800);

	pcg =  (uint8_t*)ps_malloc(0x800);
	memset(pcg, 0, 0x800);

	gram = (uint8_t*)ps_malloc(0x8000);
	memset(gram, 0, 0x8000);

	wdmy = (uint8_t*)ps_malloc(0x4000);
	rdmy = (uint8_t*)ps_malloc(0x4000);
	memset(rdmy, 0xff, 0x4000);

	kanji = (uint8_t*)ps_malloc(0x23400);
	memset(kanji, 0xff, 0x23400);
	
	text = (uint8_t*)ps_malloc(200*640);
	graph = (uint8_t*)ps_malloc(400*640);
...
{
#if defined(_SMC70)
	SET_BANK(0x0000, sizeOfROM - 1, wdmy, rom);
	SET_BANK(sizeOfROM, 0xffff, ram + sizeOfROM, ram + sizeOfROM);
#else
	SET_BANK(0x0000, sizeOfROM - 1, wdmy, rom);
	SET_BANK(sizeOfROM, 0xffff, ram + sizeOfROM, ram + sizeOfROM);
#endif
...
uint32_t MEMORY::fetch_op(uint32_t addr, int *wait)
{
	if(rom_switch_wait) {
		...
		if(--ram_switch_wait == 0) {
			SET_BANK(0x0000, sizeOfROM - 1, ram, ram);
			rom_selected = false;
		}
	}
	*wait = 0;
...
void MEMORY::event_vline(int v, int clock)
{
	if(v < 200) {
		...
		if(v == 0) {
			memset(text, 0, 200*640);
			memset(graph, 0, 400*640);
		}
		if(vsup) {
			return;
		}
...
//	#define palette_graph_pc palette_pc
	...
	for(int y = 0; y < 200; y++) {
		...
		uint8_t* src_t = text + y * 640;
		uint8_t* src_g = graph + y * 640;
#if defined(_SMC777)
...
//		}
		...
		for(int x = 0; x < 320; x++) {
			uint8_t t = src_t[x*2];
			uint8_t t2 = src_t[x*2 + 1];

			scrntype_t color1 = t ? palette_text_pc[t & 15] : palette_graph_pc[src_g[x * 2]];
			scrntype_t color2 = t2 ? palette_text_pc[t2 & 15] : palette_graph_pc[src_g[x * 2 + 1]];
...
//			for(int l = 0; l < ht; l++) {
				...
				uint8_t* d0 = graph + (2 * yy    ) * 640 + (x << 3);
				//uint8_t* d1 = &graph[2 * yy + 1][x << 3];
				uint8_t* d1 = graph + (2 * yy + 1) * 640 + (x << 3);
...
//			for(int l = 0; l < ht; l += 2) {
				...
				uint8_t* d0 = graph + yy    * 640 + (x << 3);
				//uint8_t* d1 = &graph[yy + 1][x << 3];
				uint8_t* d1 = graph + (yy + 1) * 640 + (x << 3);
				d0[0] = d0[1] = d0[2] = d0[3] = 
...
bool MEMORY::process_state(FILEIO* state_fio, bool loading)
{
	...
	state_fio->StateArray(cram, 0x800, 1);
	state_fio->StateArray(aram, 0x800, 1);
	state_fio->StateArray(pcg, 0x800, 1);
	state_fio->StateArray(gram, 0x8000, 1);
