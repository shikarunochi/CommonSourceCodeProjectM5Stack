/*
	...
	M5Stack version.
	modified by shikarunochi 2020.06.01 -
...
void MEMORY::initialize()
{
	...
	vram = (uint8_t*)ps_malloc(vramSize);
	tvram = (uint8_t*)ps_malloc(0x1000);
	ipl = (uint8_t*)ps_malloc(0x800);
	font = (uint8_t*)ps_malloc(0x800);
	screen_txt = (uint8_t*)ps_malloc(200*640);
	screen_gra = (uint8_t*)ps_malloc(200*640);
#ifdef _MZ80B
	screen_80b_green = (uint8_t*)ps_malloc(200*640);
	screen_80b_vram1 = (uint8_t*)ps_malloc(200*640);
	screen_80b_vram2 = (uint8_t*)ps_malloc(200*640);
#endif
	// memory
	...
	for(int i = 0; i < 8;i++){
		for(int j = 0;j < 8;j++){
			palette_color_half[i][j] =  RGB_COLOR(
					(((i & 2) ? 255 : 0) + ((j & 2) ? 255 : 0)) / 2, 
					(((i & 4) ? 255 : 0) + ((j & 4) ? 255 : 0)) / 2, 
					(((i & 1) ? 255 : 0) + ((j & 1) ? 255 : 0)) / 2);
		}
	}
	...
	palette_green[1] = RGB_COLOR(0, 128, 0);
	palette_green[2] = RGB_COLOR(0, 255, 0);
#endif
...
bool MEMORY::load_mzt_image(const _TCHAR* file_path)
{
	...
	if(is_mtw || ipl_selected) {
		...
		if(fio->Fopen(file_path, FILEIO_READ_BINARY)) {
			...
			Serial.println("memory load check");
			Serial.printf("offs=%d\n", offs);
//			if(header[0] == 0x01 && (is_mtw || size > 0x7e00)) {
			if(header[0] == 0x01 && offs == 0) {
				Serial.println("memory load execute");
				memset(ram, 0, 0x10000);
				memset(vram, 0, vramSize);
				memset(tvram, 0, 0x1000);
	...
	for(int y = 0, addr = 0; y < 200; y += 8) {
		for(int x = 0; x < (width80 ? 80 : 40); x++) {
			...
			for(int l = 0; l < 8; l++) {
				...
				uint8_t* d = screen_txt + (y + l) * 640 + (x << 3);
...
//	} else {
		...
		for(int y = 0, addr = 0; y < 200; y++) {
			for(int x = 0; x < 80; x++) {
				...
				addr++;
				uint8_t* d = screen_gra + y * 640 + (x << 3);
...
//	}
	...
	for(int y = 0; y < 200; y++) {
		uint8_t* dest0 = screen_80b_green + y * 640;
		//uint8_t* src_txt = screen_txt[y];
		uint8_t* src_txt = screen_txt + y * 640;
		//uint8_t* src_gra = screen_gra[y];
		uint8_t* src_gra = screen_gra + y * 640;
	...
	for(int y = 0, addr = 0; y < 200; y++) {
		for(int x = 0; x < 40; x++) {
			...
			uint8_t* d1 = screen_80b_vram1 + y * 640 + (x << 4);
			uint8_t* d2 = screen_80b_vram2 + y * 640 + (x << 4);
	...
	if(config.monitor_type != MONITOR_TYPE_GREEN) {
		...
		for(int y = 0; y < 200; y++) {
			...
			scrntype_t* dest0 = emu->get_screen_buffer(y) + offset;
			uint8_t* src_txt = screen_txt + y * 640;
			uint8_t* src_gra = screen_gra + y * 640;
			...
			if(vgate || reverse) {
				...
				if(text_color & 8) {
					// graphics > text
					for(int x = 0; x < 320; x++) {
						uint8_t txt1 = src_txt[width80 ? x * 2: ((x * 2) >> 1)], gra1 = src_gra[x * 2];
						uint8_t txt2 = src_txt[width80 ? x * 2 + 1: ((x * 2 + 1) >> 1)], gra2 = src_gra[x * 2 + 1];
						dest0[x] = palette_color_half[gra1 ? gra1 : txt1 ? (txt1 & 7) : back_color][gra2 ? gra2 : txt2 ? (txt2 & 7) : back_color];
					}
				} else {
					// text > graphics
					for(int x = 0; x < 320; x++) {
						uint8_t txt1 = src_txt[width80 ? x * 2 : ((x * 2) >> 1)], gra1 = src_gra[x * 2];
						uint8_t txt2 = src_txt[width80 ? x * 2 + 1 : ((x * 2 + 1) >> 1)], gra2 = src_gra[x * 2 + 1];
						dest0[x] = palette_color_half[txt1 ? (txt1 & 7) : gra1 ? gra1 : back_color][txt2 ? (txt2 & 7) : gra2 ? gra2 : back_color];
					}
				}
			}
			//if(config.scan_line) {
	...
	if(config.monitor_type != MONITOR_TYPE_GREEN) {
		...
		int offset = (config.monitor_type == MONITOR_TYPE_GREEN_COLOR + offset) ? 640 : 0;
		emu->set_vm_screen_lines(200);
		for(int y = 0; y < 200; y++) {
			//scrntype_t* dest0 = emu->get_screen_buffer(y * 2 + 0) + offset;
			//scrntype_t* dest1 = emu->get_screen_buffer(y * 2 + 1) + offset;
			scrntype_t* dest0 = emu->get_screen_buffer(y) + offset;
			uint8_t* v1 = screen_80b_vram1 + y * 640;
			uint8_t* v2 = screen_80b_vram2 + y * 640;
			uint8_t* gr = screen_80b_green + y * 640;
			
			//for(int x = 0; x < 640; x++) {
			//	dest0[x] = pio3039_color[pio3039_palette[(pio3039_txt_sw ? (gr[x] ? 4 : 0) : 0) + (v2[x] ? 2 : 0) + (v1[x] ? 1 : 0)]];
			//}
			for(int x = 0; x < 320; x++) {
				dest0[x] = pio3039_color[pio3039_palette[(pio3039_txt_sw ? (gr[x * 2] ? 4 : 0) : 0) + (v2[x * 2] ? 2 : 0) + (v1[x * 2] ? 1 : 0)]]
				 		| pio3039_color[pio3039_palette[(pio3039_txt_sw ? (gr[x * 2 + 1] ? 4 : 0) : 0) + (v2[x * 2 + 1] ? 2 : 0) + (v1[x * 2 + 1] ? 1 : 0)]];
			}
	...
	if(config.monitor_type != MONITOR_TYPE_COLOR) {
		...
		for(int y = 0; y < 200; y++) {
			...
			//scrntype_t* dest1 = emu->get_screen_buffer(y * 2 + 1) + offset;
			scrntype_t* dest0 = emu->get_screen_buffer(y) + offset;
			uint8_t* src_80b = screen_80b_green + y * 640;
			
			//for(int x = 0; x < 640; x++) {
			//	dest0[x] = palette_green[src_80b[x] ? 1 : 0];
			//}
			for(int x = 0; x < 320; x++) {
				dest0[x] = palette_green[(src_80b[x * 2] ? 1 : 0)
				                     + (src_80b[x * 2 + 1] ? 1 : 0)];
			}
...
bool MEMORY::process_state(FILEIO* state_fio, bool loading)
{
	...
	state_fio->StateArray(vram, vramSize, 1);
	state_fio->StateArray(tvram, 0x1000, 1);
