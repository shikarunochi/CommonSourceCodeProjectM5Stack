...
void MZ1R12::initialize()
{
	...
	address = 0;
	crc32 = get_crc32(sram, 0x8000);
}

void MZ1R12::release()
{
	if(!read_only && crc32 != get_crc32(sram, 0x8000)) {
		FILEIO* fio = new FILEIO();
		if(fio->Fopen(create_local_path(_T("MZ-1R12.BIN")), FILEIO_WRITE_BINARY)) {
			fio->Fwrite(sram, 0x8000, 1);
			fio->Fclose();
		}
		delete fio;
	}
...
bool MZ1R12::process_state(FILEIO* state_fio, bool loading)
{
	...
	state_fio->StateArray(sram, 0x8000, 1);
