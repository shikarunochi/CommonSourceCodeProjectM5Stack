...
VM::VM(EMU* parent_emu) : VM_TEMPLATE(parent_emu)
{
	...
	for(DEVICE* device = first_device; device; device = device->next_device) {
		Serial.println(device->get_device_name());
		device->initialize();
		Serial.println("DONE");
	}
	for(int i = 0; i < 4; i++) {
	...
	Serial.println("ALL INITIALIZE DONE");
}
...
{
	return false;//drec->is_tape_inserted();
}

bool VM::is_tape_playing(int drv)
{
	return false;//drec->is_tape_playing();
}

bool VM::is_tape_recording(int drv)
{
	return false;//drec->is_tape_recording();
}
...
bool VM::process_state(FILEIO* state_fio, bool loading)
{
	...
	for(DEVICE* device = first_device; device; device = device->next_device) {
		const char *name = ""; //shikarunochi typeid(*device).name() + 6; // skip "class "
		int len = strlen(name);
