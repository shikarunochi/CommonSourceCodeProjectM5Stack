...
{
	vram = (uint8_t*)ps_malloc(TMS9918A_VRAM_SIZE);
	screen = (uint8_t*)ps_malloc(192*256);//[192][256];
	collision = (uint8_t*)ps_malloc(192*256);
	// register event
...
void TMS9918A::reset()
{
	memset(vram, 0, TMS9918A_VRAM_SIZE);
...
void TMS9918A::draw_screen()
{
	...
	for(int y = 0, y2 = 0; y < 192; y++, y2 += 2) {
		...
		uint8_t* src =  screen + y * SCREEN_WIDTH;
...
void TMS9918A::event_vline(int v, int clock)
{
	if(v == 192) {
		...
		if(regs[1] & 0x40) {
			...
			memset(screen, 0, 192*256 /*sizeof(screen)*/);
		}
	...
	for(int y = 0, name = 0; y < 24; y++) {
		for(int x = 0; x < 32; x++) {
			...
			for(int yy=0; yy < 8; yy++) {
				...
				uint8_t* buffer = screen + (y * 8 + yy)*256 + x * 8;
...
void TMS9918A::draw_mode1()
{
	...
	memset(screen, bg, 192*256 /*sizeof(screen)*/);
	for(int y = 0, name = 0; y < 24; y++) {
		for(int x = 0; x < 40; x++) {
			uint16_t code = vram[name_table + (name++)];
			uint8_t* pattern_ptr = vram + pattern_table + code * 8;
			for(int yy = 0; yy < 8; yy++) {
				...
				uint8_t* buffer = screen + (y * 8 + yy)*256 + x * 6 + 8;
				buffer[0] = (pattern & 0x80) ? fg : bg;
				buffer[1] = (pattern & 0x40) ? fg : bg;
				buffer[2] = (pattern & 0x20) ? fg : bg;
				buffer[3] = (pattern & 0x10) ? fg : bg;
		...
		for(int x = 0; x < 32; x++) {
			...
			for(int yy = 0; yy < 2; yy++) {
				...
				for(int yyy = 0; yyy < 4; yyy++) {
					...
					uint8_t* buffer = screen + (y * 8 + yy * 4 + yyy) *256 + x * 8;
...
void TMS9918A::draw_sprites()
{
	...
	uint8_t limit[192];
	//uint8_t* collision = (uint8_t*)ps_malloc(192*256);これはinitで確保
	int illegal_sprite = 0, illegal_sprite_line = 255, p;
	memset(limit, 4, 192);
	memset(collision, 0, 192*256);
	status_reg = 0x80;
		...
		if(!large) {
			...
			for(int yy = y; yy < y + size; yy++) {
				...
				for(int xx = x; xx < x + size; xx++) {
					if(line & 0x8000) {
						if(0 <= xx && xx < 256) {
							if(collision[yy * 256 + xx]) {
								status_reg |= 0x20;
							} else {
								collision[yy * 256 + xx] = 1;
							}
							if(c && !(collision[yy * 256 + xx] & 2)) {
								collision[yy * 256 + xx] |= 2;
								*(screen + yy *256 + xx) = c;
							}
						}
					}
					...
					if(0 <= yy && yy <= 191) {
						...
						for(int xx = x; xx < x + size * 2; xx += 2) {
							if(line & 0x8000) {
								...
								if(0 <= xx + 1 && xx + 1 < 256) {
									if(collision[yy * 256 + xx + 1]) {
										status_reg |= 0x20;
									} else {
										collision[yy * 256 + xx + 1] = 1;
									}
									if(c && !(collision[yy * 256 + xx + 1] & 2)) {
										collision[yy * 256 + xx + 1] |= 2;
										*(screen + yy +256 + xx + 1) = c;
									}
								}
							}
							line *= 2;
...
bool TMS9918A::process_state(FILEIO* state_fio, bool loading)
{
	...
	state_fio->StateArray(vram, 192*256/*sizeof(vram)*/, 1);
