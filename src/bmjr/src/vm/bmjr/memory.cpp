...
void MEMORY::initialize()
{
	...
	basic = (uint8_t *)ps_malloc(0x3000);
	printer = (uint8_t *)ps_malloc(0x800);
	monitor = (uint8_t *)ps_malloc(0x1000);
	font = (uint8_t *)ps_malloc(0x800);
	
	wdmy = (uint8_t *)ps_malloc(0x800);
	rdmy = (uint8_t *)ps_malloc(0x800);
	
	color_table = (uint8_t *)ps_malloc(0x300);

	// initialize memory
	memset(ram, 0, 0x10000);
	memset(basic, 0xff, 0x3000);
	memset(printer, 0xff, 0x800);
	memset(monitor, 0xff, 0x1000);
	memset(rdmy, 0xff, 0x800);
	
	// load rom images
	FILEIO* fio = new FILEIO();
	if(fio->Fopen(create_local_path(_T("BASIC.ROM")), FILEIO_READ_BINARY)) {
		fio->Fread(basic, 0x3000, 1);
		fio->Fclose();
...
void MEMORY::event_frame()
{
	...
	if(key_stat[VK_RWIN    ]) key_data &= ~0x40; // �J�i�L�� -> R-Win
	if(key_stat[VK_KANA    ]) key_data &= ~0x80; // �J�i     -> �J�^�J�i�Ђ炪��
#else
	// this is same as "�����x�[�V�b�N�}�X�^�[Jr.(MB-6885)�G�~�����[�^ bm2"
	if(key_stat[0xa2]) key_data &= ~0x10; // �p��     -> L-CTRL
	if(key_stat[0xa0]) key_data &= ~0x20; // �p�L��   -> L-SHIFT
	if(key_stat[0xa1]) key_data &= ~0x40; // �J�i�L�� -> R-SHIFT
	if(key_stat[0xa3]) key_data &= ~0x80; // �J�i     -> R-CTRL
#endif
}
...
bool MEMORY::process_state(FILEIO* state_fio, bool loading)
{
	...
	state_fio->StateArray(color_table, 0x300, 1);
