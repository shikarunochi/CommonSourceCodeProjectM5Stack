...
String OSD::selectFile()
{
	...
	if(SD.exists(fileDir)==false){
		lcd.println("DIR NOT EXIST");
		lcd.printf("[SD:%s/]",fileDir.c_str());
		delay(5000);
		lcd.fillScreen(TFT_BLACK);
        draw_screen_enable();
		return "";
    ...
    while (true)
    {
        ...
        if (needRedraw == true)
        {
            ...
            for (int index = startIndex; index < endIndex + 1; index++)
            {
                ...
                {
                    lcd.println(fileList[index - 1].substring(0,26));
                }
            }
            lcd.setTextColor(TFT_WHITE);
        ...
        if (M5.BtnB.wasReleased())
        {
            ...
            else
            {
                ...
                if(SD.exists(fileDir + "/" + selectFileName)==false){
                    ...
                    lcd.println(fileList[selectIndex - 1]);
                    delay(2000);
                    lcd.fillScreen(TFT_BLACK);
                    needRedraw = true;
                }else{
