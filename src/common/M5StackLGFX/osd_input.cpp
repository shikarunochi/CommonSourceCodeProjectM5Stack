...
void OSD::update_input()
{
  ...
  if(M5.BtnA.wasReleased()){
    ...
    switch(joyPadMode){
	  ...
	  lcd.setCursor(0, 220);
    lcd.setTextSize(2);
    lcd.fillRect(0,210,320,30,TFT_BLACK);
    lcd.print(joyPadStatusInfo);
    delay(500);
    lcd.fillRect(0,210,320,30,TFT_BLACK);
  }
  */
  ...
  if(file.length() > 0){
    ...
    if(fileName.endsWith(".DSK")
      ...
      if(btnBLongPress == true && MAX_DRIVE >= 2)
      {
        ...
        lcd.println("SET FLOPPY DISK:Drive[2]");
      }else{
        vm->open_floppy_disk(0, cFileName, 0);
        lcd.println("SET FLOPPY DISK");
      }
      openFlag = true;
    }
#endif
...
void OSD::systemMenu()
{
  static String menuItem[] =
  {
   ...
   "RELOAD microSD Card",
   "JOYPAD",
#if defined(_MZ700)||defined(_MZ1200)||defined(_MZ80K)
    "PCG",
#endif
#if defined(USE_240X240LCD)
    "LCD",
#endif
   ""};
  ...
  while (true)
  {
    if (needRedraw == true)
    {
      ...
      for (int index = 0; index < menuItemCount; index++)
      {
        ...
        if( index == PCG_INDEX + usePCG && useExLcd == 1){
            switch(lcdMode){
                case INTERNAL_LCD:curItem = curItem + ": INTERNAL";break;
                case DOUBLE_LCD:curItem = curItem + ": INTERNAL & EXTERNAL";break;
                case EXTERNAL_LCD:curItem = curItem + ": EXTERNAL";break;
                case EXTERNAL_LCD_SPEED:curItem = curItem + ": EXTERNAL(SPEED)";break;
            }
        }
      ...
      lcd.drawCentreString("U P", 53, 240 - 17, 1);
      lcd.drawRect(110, 240 - 19, 100, 18, TFT_WHITE);
      lcd.drawCentreString("SELECT", 159, 240 - 17, 1);
      lcd.drawRect(220, 240 - 19, 100, 18, TFT_WHITE);
      lcd.drawCentreString("DOWN", 266, 240 - 17, 1);

      lcd.setTextSize(1);
      lcd.setCursor(0, 210);
      lcd.println(String(DEVICE_NAME) + " Emulator");
      lcd.setCursor(200, 210);
      lcd.print(getEmulatorVersion());
      needRedraw = false;
    }
    M5.update();
    if (M5.BtnA.wasReleased())
    ...
    if (M5.BtnB.wasReleased())
    {
      ...
      switch (selectIndex)
      {
        case 1:
          lcd.fillScreen(TFT_BLACK);
          lcd.setCursor(0, 0);
          lcd.print("Reset: " + String(DEVICE_NAME));
          delay(2000);
          lcd.fillScreen(TFT_BLACK);
		      draw_screen_enable();
          vm->reset();
        ...
        default:
          if(selectIndex == PCG_INDEX + usePCG && useExLcd == 1){
            lcdMode = lcdMode + 1;
            if(lcdMode > EXTERNAL_LCD_SPEED){
...
void OSD::gui_hid(const uint8_t* hid, int len)  // Parse HID event
{
...
bool kanaMode = false;
void OSD::keyboard(const uint8_t* d, int len)
{
    //updateStatusArea("");
    ...
    if (key_code != _last_key) {
      ...
      if(0 < _last_key && _last_key < 165){
        int vmKey = HidKeyMap[_last_key];
        if(vmKey > 0){
          vm->key_up(vmKey);
          key_status[vmKey] &= 0x7f;
        }
      }
      Serial.printf("keyCode:%x\n",key_code);
          if(0 < key_code && key_code < 165){
            int vmKey = HidKeyMap[key_code];
            Serial.printf("vmKeyCode:%x\n",vmKey);
            if(vmKey > 0){
              vm->key_down(vmKey, false);
              key_status[vmKey] = 0x80;
            }
          _last_key = key_code;
      } else {
          _last_key = 0;
