...
FM7_JCOMMCARD::FM7_JCOMMCARD(VM_TEMPLATE* parent_vm, EMU *parent_emu) : DEVICE(parent_vm, parent_emu)
{
	...
	prog_rom = (uint8_t *)ps_malloc(0x4000);
	dict_rom = (uint8_t *)ps_malloc(0x60000); // Is this right? Is not size 0x80000? 20180216 K.O
	p_ram = (uint8_t *)ps_malloc(0x2000);
	
	memset(prog_rom, 0xff, 0x4000);
	memset(dict_rom, 0xff, 0x60000);
	memset(p_ram, 0x00, 0x2000);
	cpu = NULL;
	//modified = true;
	firmware_ok = false;
...
void FM7_JCOMMCARD::initialize(void)
{
	...
	if(fio->Fopen(create_local_path(_T(ROM_JCOMM_DICTIONARY)), FILEIO_READ_BINARY)) {
		nsize = fio->Fread(dict_rom, 1, 0x60000);
		fio->Fclose();
		if(nsize >= 0x60000) {
...
bool FM7_JCOMMCARD::decl_state(FILEIO *state_fio, bool loading)
{
	...
	state_fio->StateArray(prog_rom, 0x4000, 1);
	state_fio->StateArray(dict_rom, 0x60000, 1);
	state_fio->StateArray(p_ram, 0x2000, 1);
