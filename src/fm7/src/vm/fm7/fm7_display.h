...
#endif
protected:
	uint32_t (DISPLAY_::*read_cpu_func_table[512])(uint32_t);
	uint32_t (DISPLAY_::*read_dma_func_table[512])(uint32_t);
	void (DISPLAY_::*write_cpu_func_table[512])(uint32_t, uint8_t);
	void (DISPLAY_::*write_dma_func_table[512])(uint32_t, uint8_t);
...
public:
	DISPLAY_(VM_TEMPLATE* parent_vm, EMU *parent_emu);
	~DISPLAY_();
