...
void FM7_MAINIO::initialize()
{
	...
	event_timerirq = -1;
	event_fdc_motor = -1;
	lpt_type = 0;//config.printer_type;
	fdc_cmdreg = 0x00;
#if defined(HAS_2HD)
...
uint8_t FM7_MAINIO::get_port_fd02(void)
{
	...
	if(lpt_type == 0) {
		lpt_busy = false;// (printer->read_signal(SIG_PRINTER_BUSY) != 0);
		lpt_error_inv = true;
		lpt_ackng_inv = true;//(printer->read_signal(SIG_PRINTER_ACK) != 0);
		lpt_pe = false;
...
//#if !defined(_FM8)			
		...
		case 0x45: // WHG CMD
			break;
		case 0x46: // WHG DATA
			retval = 0;//(uint32_t) get_opn(1);
			break;
		case 0x47:
			retval = 0;//(uint32_t) get_extirq_whg();
			break;
		case 0x51: // THG CMD
			break;
		case 0x52: // THG DATA
			retval = 0;//(uint32_t) get_opn(2);
			break;
		case 0x53:
			retval = 0;//(uint32_t) get_extirq_thg();
			break;
//#endif			
#if defined(HAS_MMR)
		case 0x93:
...
{
	//Serial.printf("MAINIO:write data8:%X, %X\n",addr,data);
	bool flag;
	uint32_t mmr_segment;
	if(addr < FM7_MAINIO_IS_BASICROM) {
		addr = addr & 0xff;
...
# endif
	...
	//Serial.printf("MAINIO:write data8:END\n");
}

void FM7_MAINIO::event_callback(int event_id, int err)
{
	//Serial.printf("MAIN EVENT id=%d\n", event_id);
