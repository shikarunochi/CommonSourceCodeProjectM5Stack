...
void DISPLAY_::draw_screen2()
{
	//Serial.println("DrawScreen2:Start");
	int y;
	int x;
	scrntype_t *p, *pp, *p2;
	int yoff;
...
bool DISPLAY_::screen_update(void)
{
	if(crt_flag) {
...
void DISPLAY_::reset_screen_update(void)
{
	screen_update_flag = false;
}

void DISPLAY_::GETVRAM_8_200L(int yoff, scrntype_t *p,
							 scrntype_t *px,
							 bool window_inv,
							 bool scan_line)
{
...
__DECL_VECTORIZED_LOOP
	for(int i = 0; i < 4; i++) {
		scrntype_t color1 = dpalette_pixel[tmp_d[i * 2]];
		scrntype_t color2 = dpalette_pixel[tmp_d[i * 2 + 1]]; 
		...
		tmp_dd[i] = RGB_COLOR(rr,gg,bb);
		//Serial.printf("i=%d  tmp_d[i]=%X tmp_dd[i]=%X \n ",i,tmp_d[i],tmp_dd[i]);
	}
#if defined(FIXED_FRAMEBUFFER_SIZE)
	if(scan_line) {
/* Fancy scanline */
...
#if defined(_FM77L4)
scrntype_t DISPLAY_::GETVRAM_TEXTCOLOR(uint8_t attr, bool do_green)
{
	int color = attr & 0x07;
	int r, g, b;
...
uint8_t DISPLAY_::GETVRAM_TEXTPIX(uint8_t bitdata, bool reverse, bool cursor_rev)
{
	uint8_t ret = bitdata;
	if(reverse) {
		ret = (uint8_t)(~ret);
...
void DISPLAY_::GETVRAM_1_400L(int yoff, scrntype_t *p)
{
	uint8_t pixel;
	uint32_t yoff_d;
	if(p == NULL) return;
...
void DISPLAY_::GETVRAM_1_400L_GREEN(int yoff, scrntype_t *p)
{
	uint8_t pixel;
	uint32_t yoff_d;
	if(p == NULL) return;
...
void DISPLAY_::GETVRAM_8_200L_GREEN(int yoff, scrntype_t *p,
							 scrntype_t *px,
							 bool window_inv,
							 bool scan_line)
{
...
#if defined(_FM77AV_VARIANTS)
void DISPLAY_::GETVRAM_4096(int yoff, scrntype_t *p, scrntype_t *px,
						   uint32_t mask,
						   bool window_inv,
						   bool scan_line)
{
...
#if defined(_FM77AV40) || defined(_FM77AV40EX) || defined(_FM77AV40SX)
void DISPLAY_::GETVRAM_8_400L(int yoff, scrntype_t *p,
							 bool window_inv)
{
	uint8_t b, r, g;
	uint32_t dot;
...
void DISPLAY_::GETVRAM_256k(int yoff, scrntype_t *p, scrntype_t *px, bool scan_line)
{
	uint32_t b3, r3, g3;
	uint32_t b4, r4, g4;
