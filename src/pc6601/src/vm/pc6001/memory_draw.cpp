...
#define SETSCRVARM1(y1)	dest = screen + y1 * 320 + 0;W=0;
#define SETSCRVARM5(y1)	dest = screen + y1 * 320 + 0;W=0;
#define M1HEIGHT 192
#define M5HEIGHT 200
#define M6HEIGHT 204
...
void MEMORY::draw_screen()
{
	if (CRTKILL) {
		...
		for(int y = 0; y < 200; y++) {
			scrntype_t* dest = emu->get_screen_buffer(y);
			memset(dest, 0, 320 * sizeof(scrntype_t));
		}
		...
		if (CRTMode1) {
			...
			for(int y = 0; y < 200; y++) {
				...
				//scrntype_t* dest1 = emu->get_screen_buffer(y*2+1);
				for(int x = 0; x < 320; x++) {
					//dest[x*2] = dest[x*2+1] = palette_pc[screen[y * 320 + x]];
					dest[x] = palette_pc[screen[y * 320 + x]];
				}
				//if(config.scan_line) {
				//	memset(dest1, 0, 640 * sizeof(scrntype_t));
				//} else {
			...
			for(int y = 0; y < 200; y++) {
				...
				for(int x = 0; x < 320; x++) {
					...
					//	dest[x*2] = dest[x*2+1] = palette_pc[screen[(y-4) * 320 + x-32]];
						dest[x] = palette_pc[screen[(y-4) * 320 + x-32]];
					} else {
...
{
	if (port60[ (A>>13)+flag ]==0x00 && bitmap)	// VRAM �̐擪���ACRT�� BITMAP mode
		return 1;
	return 0;
}
...
uint8_t MEMORY::gvram_read(uint32_t A)
{
	...
	x = A & 0x1fff;
	y = portCF*16+portCE;		      /* y���W */
	if( y >=204) y-=204;		      /* Y���W 204 �ȏゾ�� 204 ���� add 2003/10/22 */
	w = (x <256) ? 256: 64;          /* width:0..255 �Ȃ�256 / 256..319�Ȃ� 64�ɂ���*/
	off=(x <256) ? 0x1a00: 0x0000;   /* offset: Vram offset address */
	x = (x <256) ? x: x-256;	      /* x:256..319 �Ȃ� 256�������@ */
	z = ((y & 1 )==1) ? 2: 0;        /* z:Y���W����Ȃ�A2�𑫂�  */
