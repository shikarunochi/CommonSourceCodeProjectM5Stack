...
void VDP::initialize()
{
    text = (uint8_t*)ps_malloc(320*320);
	sprite = (uint8_t*)ps_malloc(320*320);
	font = (uint8_t*)ps_malloc(128*8);
	...
	for(int i = 0, c = 0x20, p = 0; i < 3; i++, c += 32) {
		for(int j = 0; j < 16; j++) {
			for(int k = 0; k < 8; k += 2) {
				...
				*(font + (c + j) * 8 + k)  = (d0 & 0xf0) | (d1 >> 4);
				*(font + (c + j + 16) * 8 + k) = (d2 & 0xf0) | (d3 >> 4);
				*(font + (c + j) * 8 + k + 1) = (d0 << 4) | (d1 & 0x0f);
				*(font + (c + j + 16) * 8 + k + 1) = (d2 << 4) | (d3 & 0x0f);
			}
		}
	}
...
void VDP::draw_screen()
{
	...
	// draw text screen
	memset(text, vdc1 & 0xf, 320*320);
	draw_text_screen();
	
	// draw sprite screen
	memset(sprite, 0, 320*320);
	if(vdc0 & 0x10) {
		draw_sprite_screen();
	}
...
void VDP::draw_text(int dx, int dy, uint8_t data, uint8_t tcol, uint8_t bcol)
{
	...
	for(int l = 0; l < 8 && data; l++) {
		...
		uint8_t* dest = text + (dy16 + l) * 320 + dx8;
		//uint8_t pat = font[data][l];
		uint8_t pat = *(font + (data * 8) + l);
	...
	for(int l = (data ? 8 : 0); l < 16; l++) {
		...
		memset(text + (dy16 + l) * 320 + dx8, bcol, 8);
	}
}
...
void VDP::draw_block(int dx, int dy, uint8_t data)
{
	...
	if(cu) {
		for(int l = 0; l < 8; l++) {
			...
			memset(text + (dy16 + l) * 320 + dx8, cu, 8);
		}
	}
	if(cl) {
		for(int l = 8; l < 16; l++) {
			//memset(&text[dy16 + l][dx8], cl, 8);
			memset(text + (dy16 + l) * 320 + dx8, cl, 8);
		}
	}
}
...
void VDP::draw_graph(int dx, int dy, uint8_t data, uint8_t col)
{
	...
	if(data & 0x80) {
		for(int l = 0; l < 4; l++) {
			...
			memset(text + (dy16 + l) * 320 + dx8l, col, 4);
		}
	}
	if(data & 0x40) {
		for(int l = 0; l < 4; l++) {
			//memset(&text[dy16 + l][dx8r], col, 4);
			memset(text + (dy16 + l) * 320 + dx8r, col, 4);
		}
	}
	if(data & 0x20) {
		for(int l = 4; l < 8; l++) {
...
void VDP::draw_sprite(int dx, int dy, int sx, int ex, int sy, int ey, int no, uint8_t col)
{
	...
	for(int y = (sy < 0 ? 0 : sy), no32 = no << 5; y < ey; y++) {
		...
		for(int x = sx; x < ex; x++) {
			...
			uint8_t* du = sprite + y2u * 320 + x4;
			//uint8_t* dl = &sprite[y2l][x4];
			uint8_t* dl = sprite + y2l * 320 + x4;
