...
{
	cart = (uint8_t *)ps_malloc(0x8000*4);
	sram = (uint8_t *)ps_malloc(0x2000);
	//bios = (uint8_t *)ps_malloc(0x1000);
	//vram = (uint8_t *)ps_malloc(0x2000);


	memset(bios, 0xff, 0x1000);
	memset(cart, 0xff, 0x8000*4);
	memset(sram, 0xff, 0x2000);
...
void MEMORY::open_cart(const _TCHAR* file_path)
{
	...
	if(fio->Fopen(file_path, FILEIO_READ_BINARY)) {
		...
		if(header.ctype == 1 && cart[1] != 0x48) {
			...
			sram_crc32 = get_crc32(sram, 0x2000);
		}
		inserted = true;
	}
	delete fio;
...
void MEMORY::close_cart()
{
	...
	if(inserted && header.ctype == 1 && cart[1] != 0x48) {
		if(sram_crc32 != get_crc32(sram, 0x2000)) {
			FILEIO* fio = new FILEIO();
...
bool MEMORY::process_state(FILEIO* state_fio, bool loading)
{
	...
	state_fio->StateArray(vram, 0x2000, 1);
	state_fio->StateArray(wreg, sizeof(wreg), 1);
	state_fio->StateArray(sram, 0x2000, 1);
