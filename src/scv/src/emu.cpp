...
#endif
{
	Serial.println("common initialize");
	common_initialize();
#ifdef _DEBUG_LOG
...
//		config.sound_frequency = 6;	// default: 48KHz
	...
	Serial.println("OSD initial OK");
	// initialize vm
	osd->vm = vm = new VM(this);
	Serial.println("VM OK");
#if defined(_USE_QT)
	osd->reset_vm_node();
#endif
#ifdef USE_AUTO_KEY
	...
	initialize_media();
	Serial.println("initialize Media OK");
...
static bool hex2bin(const _TCHAR* file_path, const _TCHAR* dest_path)
{
	...
	if(fio_s->Fopen(file_path, FILEIO_READ_BINARY)) {
		int length = 0;
		char *line = (char*)ps_malloc(1024);
		uint8_t *buffer = (uint8_t *)ps_malloc(0x10000);
