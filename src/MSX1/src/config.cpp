...
void load_config(const _TCHAR* config_path)
{
	...
	#ifdef USE_FLOPPY_DISK
		for(int drv = 0; drv < USE_FLOPPY_DISK; drv++) {
			config.correct_disk_timing[drv] = true;//MyGetPrivateProfileBool(_T("Control"), create_string(_T("CorrectDiskTiming%d"), drv + 1), config.correct_disk_timing[drv], config_path);
			config.ignore_disk_crc[drv] = true;//MyGetPrivateProfileBool(_T("Control"), create_string(_T("IgnoreDiskCRC%d"), drv + 1), config.ignore_disk_crc[drv], config_path);
		}
	#endif
	#ifdef USE_TAPE
		for(int drv = 0; drv < USE_TAPE; drv++) {
			config.wave_shaper[drv] = true;//MyGetPrivateProfileBool(_T("Control"), create_string(_T("WaveShaper%d"), drv + 1), config.wave_shaper[drv], config_path);
			config.direct_load_mzt[drv] = true;//MyGetPrivateProfileBool(_T("Control"), create_string(_T("DirectLoadMZT%d"), drv + 1), config.direct_load_mzt[drv], config_path);
			config.baud_high[drv] = true;//MyGetPrivateProfileBool(_T("Control"), create_string(_T("BaudHigh%d"), drv + 1), config.baud_high[drv], config_path);
		}
	#endif
	config.compress_state = true;//MyGetPrivateProfileBool(_T("Control"), _T("CompressState"), config.compress_state, config_path);
...
//		#endif
	...
	#ifdef USE_FLOPPY_DISK
		config.sound_noise_fdd = false;//MyGetPrivateProfileBool(_T("Sound"), _T("NoiseFDD"), config.sound_noise_fdd, config_path);;
	#endif
	#ifdef USE_TAPE
		config.sound_noise_cmt = false;//MyGetPrivateProfileBool(_T("Sound"), _T("NoiseCMT"), config.sound_noise_cmt, config_path);;
		config.sound_play_tape = false;//MyGetPrivateProfileBool(_T("Sound"), _T("PlayTape"), config.sound_play_tape, config_path);
	#endif
