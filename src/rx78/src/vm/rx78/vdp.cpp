...
void VDP::initialize()
{
	screen0 = (uint8_t *)ps_malloc(184 * 192);
	screen1 = (uint8_t *)ps_malloc(184 * 192);
...
void VDP::draw_screen()
{
	...
	for(int y = 0, src = 0; y < 184; y++) {
		uint8_t* dest0 = screen0 + y * 192;
		uint8_t* dest1 = screen1 + y * 192;
	...
	for(int y = 0; y < 184; y++) {
		...
		uint8_t* src0 = screen0 + y * 192;
		uint8_t* src1 = screen1 + y * 192;
