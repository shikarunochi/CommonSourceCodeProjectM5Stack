...
VM::VM(EMU* parent_emu) : VM_TEMPLATE(parent_emu)
{
	...
	ipl = (uint8_t*)ps_malloc(0x2000);	// ipl (8k)
	cart = (uint8_t*)ps_malloc(0x5000);	// cartridge (20k)
	ram = (uint8_t*)ps_malloc(0x1000);	// ram (4k)
	ext = (uint8_t*)ps_malloc(0x8000);	// ext ram (32k)
	...
	memset(cart, 0xff, 0x5000);
	
	memory->read_bios(_T("IPL.ROM"), ipl, 0x2000);
...
void VM::open_cart(int drv, const _TCHAR* file_path)
{
	if(drv == 0) {
		memset(cart, 0xff, 0x5000);
		inserted = memory->read_image(file_path, cart, 0x5000);
		reset();
	}
}
...
bool VM::process_state(FILEIO* state_fio, bool loading)
{
	...
	state_fio->StateArray(ram, 0x1000, 1);
	state_fio->StateArray(ext, 0x8000, 1);
