...
private:
	...
	uint8_t *tapeData;
	uint8_t getNextTapeBuffer();
	int fetchNextTapeBuffer();
	void setOnBit();
	void setOffBit();
	void setTapePos(int startPos, int endPos);
	int8_t getNextTapeBit();
	int onBitCount;
	int offBitCount;
	int tapeLoadPhase;
	int bitLength;
	...
	int tapeCheckCount;
	int tapeLength;
	int signalCheckCount;

	int curFileSize;
	int curFileTapeStartPos;
public:
...
#endif
		my_tcscpy_s(message, 128, _T("Stop"));
		drive_num = 0;
