...
void MEMORY::initialize()
{
	...
	gal5_screen = (uint8_t *)ps_malloc(200 * 320);
...
uint32_t MEMORY::read_data8(uint32_t addr)
{
	...
	if(0xe000 <= addr && addr <= 0xe7ff) {
		...
		case 0xe000: case 0xe001: case 0xe002: case 0xe003:
		{
			uint32_t temp_result = d_pio->read_io8(addr & 3);
			if(addr == 0xE002){
				//Serial.printf("MEMORY::read_data8:case[%X] %X\n",addr, temp_result);
...
void MEMORY::draw_screen()
{
	...
	if((config.monitor_type & 3) != MONITOR_TYPE_MONOCHROME) {
		for(int y = 0; y < 200; y++) {
			...
			uint8_t* srcg = gal5_screen + y*320;
			for(int x = 0; x < 320; x++) {
...
void MEMORY::draw_line(int v)
{
	...
	for(int x = 0; x < 320; x += 8) {
		...
		uint8_t* dtc = gal5_screen + v * 320 + x;
#endif
...
void MEMORY::memoryDump(){
	for(int i = 0x10f0;i < 0x1170;i++){
		//Serial.printf("[%X] %X\n", i, ram[i]);
		Serial.printf("%0X:", ram[i]);
	}
	Serial.println();
}
