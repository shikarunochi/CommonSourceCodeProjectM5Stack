...
void WAVFILE::initialize(FILEIO *parent_play_fio){
    ...
    while(dataCheck == false){
        ...
        readCount++;
        for(int index = 0;index  <TAPBUFFERSIZE;index++){
            if(tapeBuffer[index]!=0){
                dataCheck=true;
                break;
            }
        }
    }
    tapePosition = tapePosition + TAPBUFFERSIZE * (readCount - 1);
    preCheckSampleSignal =  (tapeBuffer[bufferPosition] - 128) * 256;
    bufferPosition++;
    tapePosition++;
    sampleSignal =  (tapeBuffer[bufferPosition] - 128) * 256;
    bufferPosition++;
    tapePosition++;
}
